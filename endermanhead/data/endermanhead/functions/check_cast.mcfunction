...
execute if entity @e[type=minecraft:armor_stand,nbt={ArmorItems:[{id:"minecraft:player_head",tag:{SkullOwner:"MHF_Enderman"}}]},distance=..15] run tag @s add run_cast
execute if entity @a[tag=!is_me,distance=..15,nbt={Inventory:[{Slot:103b,id:"minecraft:player_head",tag:{SkullOwner:{Name:"MHF_Enderman"}}}]}] run tag @s add run_cast
